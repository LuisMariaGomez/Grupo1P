    ...
    public partial class CU_002___TipoDoc___Armado_Interfaz : Form
    {
        ...
        private void dataGridView_TipoDocumento_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e != null && dataGridView_TipoDocumento.Rows.Count > 0)
            {
                ...
                label_codigoDocumento.Text = dataGridView_TipoDocumento[1, e.RowIndex].Value.ToString();
                label_TipoDocumento.Text =
                    (dataGridView_TipoDocumento[0, e.RowIndex].Value != null)
                        ? dataGridView_TipoDocumento[0, e.RowIndex].Value.ToString()
                        : "";
            }
            else
            {
        ...
        private void ObtieneDatosDesdeRepositorio()
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    var lista = Negocio.CasoDeUso.Personas.ObtenerTodo().GetAwaiter().GetResult();
